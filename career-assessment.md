...
description: "A fun, gamified career analysis to find your perfect courses and colleges."
---
...
<style>
  ...
  .progress-bar::after { content: ''; position: absolute; top:0; left:0; right:0; bottom:0; background: rgba(255,255,255,0.2); border-radius: 50px; animation: shimmer 2s infinite; }
  ...
  .form-input, .form-textarea {
    ...
    font-size: 1.1rem; color: white; background: #0f172a; transition: all 0.3s;
    font-family: inherit;
  }
  ...
  /* EMOJI RATING SCALES - GAMIFIED */
  .rating-grid { display: flex; flex-direction: column; gap: 15px; margin-bottom: 30px; }
  .rating-row {
    display: flex; align-items: center; justify-content: space-between;
    background: #0f172a; padding: 15px 20px; border-radius: 16px; border: 1px solid var(--border-color);
    transition: 0.3s;
  }
  ...
  .option-card {
    ...
    transition: all 0.3s; background: #0f172a; display: flex; flex-direction: column; align-items: center; text-align: center; position: relative; overflow: hidden;
  }
  .option-card:hover { border-color: var(--secondary); transform: translateY(-5px); box-shadow: 0 10px 20px rgba(6, 182, 212, 0.2); }
  .option-card.selected { border-color: var(--primary); background: rgba(139, 92, 246, 0.1); box-shadow: 0 0 0 2px var(--primary); }
  .opt-icon { font-size: 2.5rem; margin-bottom: 10px; z-index: 1; }
  .opt-title { font-weight: 800; color: white; font-size: 1.05rem; z-index: 1; }
  ...
  .btn-main {
    ...
    font-size: 1.1rem; font-weight: 900; border-radius: 50px; cursor: pointer; flex-grow: 1; transition: 0.3s;
    text-transform: uppercase; letter-spacing: 1px; box-shadow: 0 10px 20px rgba(139, 92, 246, 0.3);
  }
  .btn-main:hover { transform: translateY(-3px); box-shadow: 0 15px 25px rgba(139, 92, 246, 0.5); }
  .btn-back { background: transparent; border: 2px solid var(--border-color); color: var(--text-muted); flex-grow: 0; box-shadow: none; }
  .btn-back:hover { background: var(--border-color); color: white; transform: translateY(0); box-shadow: none; }

  /* DYNAMIC TOAST NOTIFICATION */
  #toast {
    position: fixed; bottom: -100px; left: 50%; transform: translateX(-50%);
    background: var(--accent); color: white; padding: 12px 24px; border-radius: 50px;
    font-weight: bold; box-shadow: 0 10px 20px rgba(0,0,0,0.3); z-index: 9999;
  ...
  .badge-code { 
    ...
    font-size: 1.8rem; margin-top: 20px; letter-spacing: 3px; box-shadow: 0 10px 20px rgba(139, 92, 246, 0.3);
  }
  ...
  .rec-card p { margin: 0; color: #64748b; font-size: 0.95rem; line-height: 1.5;}
  ...
  <div id="wizardContainer">
    ...
    <div class="step-card active" id="step1">
      ...
      <div class="form-group">
        ...
        <div class="options-grid">
          ...
          <div class="option-card" onclick="selectRadio('qLevel', 'Undergraduate')"><div class="opt-icon">🎓</div><div class="opt-title">Undergrad</div></div>
        </div>
      </div>
      <button class="btn-main" style="width:100%" onclick="nextStep(2)">Start Quest ➔</button>
    </div>
    ...
    <div class="step-card" id="step2">
            ...
            document.write(`
              <div class="rating-row" data-category="${sub.type}">
                ...
                <div class="rating-options">
                  ${[1,2,3,4,5].map(n => `<div class="rate-btn" onclick="setRating(this, '${sub.id}', ${n}, 'Subject logged!')" title="${n}/5">${emojis[n-1]}</div>`).join('')}
                </div>
              </div>
            `);
          });
      ...
      <div class="btn-nav-wrapper">
        ...
        <button class="btn-main" onclick="nextStep(3)">Next Level ➔</button>
      </div>
    </div>
    ...
    <div class="step-card" id="step3">
          ...
          acts1.forEach(act => {
            document.write(`
              <div class="rating-row" data-category="${act.type}">
                ...
                <div class="rating-options">
                  ${[1,2,3,4,5].map(n => `<div class="rate-btn" onclick="setRating(this, '${act.id}', ${n}, 'Good choice!')" title="${n}/5">${emojis[n-1]}</div>`).join('')}
                </div>
              </div>
            `);
          });
      ...
      <div class="btn-nav-wrapper">
        ...
        <button class="btn-main" onclick="nextStep(4)">Next Level ➔</button>
      </div>
    </div>
    ...
    <div class="step-card" id="step4">
      ...
      <div class="form-group">
        ...
        <div class="options-grid">
          <div class="option-card" onclick="selectRadio('p_decide', 'I_C', 'Ah, an analytical thinker! 🧠')"><div class="opt-icon">🤖</div><div class="opt-title">Pure Logic & Facts</div></div>
          <div class="option-card" onclick="selectRadio('p_decide', 'S_A', 'Empathy is a superpower! ✨')"><div class="opt-icon">❤️</div><div class="opt-title">Gut Feelings & Empathy</div></div>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="options-grid">
          <div class="option-card" onclick="selectRadio('p_task', 'C', 'Structure brings peace. 📐')"><div class="opt-icon">📏</div><div class="opt-title">Structured & Clear</div></div>
          <div class="option-card" onclick="selectRadio('p_task', 'A', 'Rules are meant to be broken! 🌊')"><div class="opt-icon">🌊</div><div class="opt-title">Flexible & Open-ended</div></div>
        </div>
      </div>

      <div class="btn-nav-wrapper">
        <button class="btn-main btn-back" onclick="prevStep(3)">Back</button>
        <button class="btn-main" onclick="nextStep(5)">Next Level ➔</button>
      </div>
    </div>
    ...
    <div class="step-card" id="step5">
          ...
          envs.forEach(env => {
            document.write(`
              <div class="rating-row" data-category="${env.type}">
                ...
                <div class="rating-options">
                  ${[1,2,3,4,5].map(n => `<div class="rate-btn" onclick="setRating(this, '${env.id}', ${n}, 'Map updated!')" title="${n}/5">${emojis[n-1]}</div>`).join('')}
                </div>
              </div>
            `);
          });
      ...
      <div class="btn-nav-wrapper">
        ...
        <button class="btn-main" onclick="nextStep(6)">Next Level ➔</button>
      </div>
    </div>
    ...
    <div class="step-card" id="step6">
      ...
      <div class="btn-nav-wrapper">
        ...
        <button class="btn-main" onclick="nextStep(7)">Final Boss ➔</button>
      </div>
    </div>
    ...
    <div class="step-card" id="step7">
      ...
      <div class="form-group">
        ...
        <input type="text" id="secretCareer" class="form-input" placeholder="e.g., Pilot, Musician, Gamer, Actor...">
      </div>

      <div class="btn-nav-wrapper">
        <button class="btn-main btn-back" onclick="prevStep(6)">Back</button>
        <button class="btn-main" style="background: var(--accent);" id="analyzeBtn" onclick="generateRIASECReport()">Decode My Brain ✨</button>
      </div>
    </div>
  ...
  <div id="reportContainer">
    <div class="report-card" id="reportContent">
      ...
      <div class="report-section avoid-section">
        ...
        <p>Based on your brain's wiring, you may experience burnout or frustration in these environments:</p>
        <p><strong id="outAvoid"></strong></p>
      </div>

      <div class="report-section" style="border-left-color: var(--secondary);">
        <h3>🏛️ Top Verified College Basecamps</h3>
        <p>Based on our directory, here are excellent institutions for your profile:</p>
        <div class="college-recs" id="outColleges"></div>
      </div>
      
      <div style="text-align: center; font-size: 0.85rem; color: #a0aec0; margin-top: 40px; border-top: 1px solid #e2e8f0; padding-top: 20px;">
        Powered by The Knowledge Habitat Career Guidance Engine.
      </div>
    </div>
...
<script>
  ...
  function showToast(msg) {
    ...
    setTimeout(() => { toast.style.bottom = "-100px"; }, 2000);
  }

  // UI HELPERS
  function setRating(btn, id, value, msg) {
  ...
  function selectRadio(group, val, msg) {
  ...
  function toggleStrength(card, val) {
    if(card.classList.contains('selected')) {
      ...
      if(state.strengths.length >= 4) { showToast("You can only select 4 max!"); return; }
      card.classList.add('selected');
      state.strengths.push(val);
      showToast("Skill Equipped! ⚔️");
    }
  }
  ...
  function generateRIASECReport() {
    ...
    setTimeout(() => {
      ...
      // Parental Pressure Correction Factor
      ...
      // Ensure no negative scores for the chart
      ...
      radarChartInstance = new Chart(ctx, {
        ...
        options: {
          scales: { r: { angleLines: { color: 'rgba(0,0,0,0.1)' }, grid: { color: 'rgba(0,0,0,0.1)' }, pointLabels: { font: { size: 14, family: 'Nunito', weight: 'bold' }, color: '#1e293b' }, ticks: { display: false } } },
  ...
  function downloadPDF() {
    ...
    // Scroll to top to prevent html2canvas blank page bug
    window.scrollTo(0,0);
